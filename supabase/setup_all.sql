-- Restaurant POS SaaS complete Supabase setup.
-- Run this ONE file in Supabase SQL Editor.
-- Safe to rerun for V1 development.
...
create trigger touch_menu_items_updated_at before update on public.menu_items
...
create trigger touch_restaurant_settings_updated_at before update on public.restaurant_settings
for each row execute function public.touch_updated_at();
...
begin
  ...
  select p.role into v_cashier_role
  from public.profiles p
  where p.id = p_cashier_id and p.restaurant_id = p_restaurant_id;
  ...
  v_bill_number := concat(
    coalesce((select rs.bill_prefix from public.restaurant_settings rs where rs.restaurant_id = p_restaurant_id), 'TS'),
    '-',
  ...
  insert into public.orders (
    restaurant_id, bill_number, cashier_id, status, subtotal, discount, tax, total
  )
  values (
    p_restaurant_id, v_bill_number, p_cashier_id, 'paid', v_subtotal, v_discount, v_tax, v_total
  )
  ...
  loop
    ...
    values (
      p_restaurant_id, v_order_id, v_menu_item_id, v_menu_item_name, v_menu_item_price,
      v_quantity, v_menu_item_price * v_quantity, v_note
    );
  end loop;
  ...
  loop
    ...
    where ii.id = v_required.inventory_item_id and ii.restaurant_id = p_restaurant_id
    for update;
    ...
    if v_inventory.quantity_on_hand < v_required.required_quantity then
      raise exception 'Insufficient stock for %. Required %, available %',
        v_inventory.name, v_required.required_quantity, v_inventory.quantity_on_hand;
    end if;
    ...
    insert into public.inventory_movements (
      restaurant_id, inventory_item_id, order_id, movement_type,
      quantity_delta, quantity_after, note, created_by
    )
    values (
      p_restaurant_id, v_required.inventory_item_id, v_order_id, 'sale_deduction',
      -v_required.required_quantity, v_inventory.quantity_on_hand,
      concat('Auto deduction for bill ', v_bill_number), p_cashier_id
    );
  end loop;

  insert into public.payments (restaurant_id, order_id, method, amount)
  values (p_restaurant_id, v_order_id, p_payment_method, v_total);

  insert into public.daily_sales_summary (
    restaurant_id, sales_date, order_count, gross_sales, discount_total, tax_total, net_sales
  )
  values (p_restaurant_id, v_sales_date, 1, v_subtotal, v_discount, v_tax, v_total)
...
begin
  for policy_record in
    ...
    where schemaname = 'public'
      and tablename in (
        'restaurants', 'profiles', 'menu_categories', 'menu_items', 'inventory_items',
        'menu_item_ingredients', 'orders', 'order_items', 'payments', 'inventory_movements',
        'daily_sales_summary', 'staff_invites', 'restaurant_settings'
      )
  loop
    execute format(
...
create policy "restaurants_select_members" on public.restaurants for select
...
create policy "profiles_select_members" on public.profiles for select
...
create policy "menu_categories_select_members" on public.menu_categories for select
using (public.same_restaurant(restaurant_id));
create policy "menu_categories_insert_manager" on public.menu_categories for insert
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "menu_categories_update_manager" on public.menu_categories for update
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']))
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "menu_categories_delete_manager" on public.menu_categories for delete
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));

create policy "menu_items_select_members" on public.menu_items for select
using (public.same_restaurant(restaurant_id));
create policy "menu_items_insert_manager" on public.menu_items for insert
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "menu_items_update_manager" on public.menu_items for update
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']))
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "menu_items_delete_manager" on public.menu_items for delete
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));

create policy "inventory_items_select_manager" on public.inventory_items for select
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "inventory_items_insert_manager" on public.inventory_items for insert
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "inventory_items_update_manager" on public.inventory_items for update
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']))
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "inventory_items_delete_manager" on public.inventory_items for delete
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));

create policy "menu_item_ingredients_select_manager" on public.menu_item_ingredients for select
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "menu_item_ingredients_insert_manager" on public.menu_item_ingredients for insert
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "menu_item_ingredients_update_manager" on public.menu_item_ingredients for update
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']))
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "menu_item_ingredients_delete_manager" on public.menu_item_ingredients for delete
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));

create policy "orders_select_members" on public.orders for select
using (public.same_restaurant(restaurant_id));
create policy "order_items_select_members" on public.order_items for select
using (public.same_restaurant(restaurant_id));
create policy "payments_select_members" on public.payments for select
using (public.same_restaurant(restaurant_id));
create policy "inventory_movements_select_manager" on public.inventory_movements for select
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));
create policy "daily_sales_summary_select_manager" on public.daily_sales_summary for select
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner','manager']));

create policy "staff_invites_select_owner" on public.staff_invites for select
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner']));
create policy "staff_invites_insert_owner" on public.staff_invites for insert
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner']));
create policy "staff_invites_update_owner" on public.staff_invites for update
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner']))
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner']));
create policy "staff_invites_delete_owner" on public.staff_invites for delete
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner']));

create policy "restaurant_settings_select_owner" on public.restaurant_settings for select
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner']));
create policy "restaurant_settings_insert_owner" on public.restaurant_settings for insert
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner']));
create policy "restaurant_settings_update_owner" on public.restaurant_settings for update
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner']))
with check (public.same_restaurant(restaurant_id) and public.has_role(array['owner']));
create policy "restaurant_settings_delete_owner" on public.restaurant_settings for delete
using (public.same_restaurant(restaurant_id) and public.has_role(array['owner']));
...
values ('11111111-1111-1111-1111-111111111111', 'TS', 'Thank you for dining with us.', true)
